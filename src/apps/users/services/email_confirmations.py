...
from django.core.signing import BadSignature, JSONSerializer, TimestampSigner
...
class EmailConfirmations:
    ...
    def send_to_user_email_confirmation_token(self, user: User) -> None:
        ...
        """
        email_confirmation_token = self._get_email_confirmation_token_for_user(user)

        send_email_address_confirmation.delay(
            recipient_list=[user.email], confirmation_token=email_confirmation_token
        )

    def _get_email_confirmation_token_for_user(self, user: User):
        """Возвращает токен для подтверждения email аддреса пользователя"""
        return TimestampSigner(salt=self._salt).sign_object(user.email, JSONSerializer)
    ...
    def _validate_user_email_confirmation_token(self, user, token: str) -> bool:
        ...
        try:
            ...
            raise ValidationError("Email address is invalid or already confirmed")
        else:
            return True


email = EmailConfirmations()
