...
def send_email_address_confirmation(
    ...
    context = {
        'email_confirmation_url':
            f'{settings.CLIENT_BASE_URL}/email-confirmation/{confirmation_token}/'

    }

    email_confirmation_template = render_to_string(
        'email-confirmation.html',
        context
    )
    with get_connection() as connection:
        email = EmailMultiAlternatives(
            subject='Welcome', from_email=settings.EMAIL_SENT_FROM,
...
def send_reset_password(
        recipient_list: List[str], reset_token: str
) -> None:
    """ Отправляет письмо пользователю на восстановление пароля """

    context = {
        'password_reset_token':
            f'{settings.CLIENT_BASE_URL}/reset-password/{reset_token}/'
    }
    password_reset_token_template = render_to_string(
        'reset_password.html',
        context
    )
    with get_connection() as connection:
