...
from django.db.models import Avg, F, Count, QuerySet, Prefetch, Window
...
class TourCRUDService(metaclass=Singleton):
    tour_manager = Tour.objects

    def _get_all_tours_info(self) -> QuerySet:
        """

        Возвращает полную информацию турах для представления
        на главной странице
        """
        tours = self.tour_manager.prefetch_related(
             'locations', 'start_locations', 'start_dates'
        ).only().annotate(
    ...
    @property
    def client_presentation_tours(self) -> QuerySet:
        """
        Возвращает 9 наиболее популярных туров для представления пользователю
        """
        return self._get_all_tours_info().order_by(
            '-ratings_quantity', '-ratings_avg'
        )[:9]
    ...
    def _get_lookup_fields(
        ...
        return {
            field: params[field] for field in params if field in lookup_keys
        }
    ...
    def get_complete_tour_info(self, params: LookupField) -> Tour:
        ...
        queryset = self._get_all_tours_info().prefetch_related(
            guides, 'images',
        ).get(**lookup_fields)
        return queryset
    ...
    def get_tour(self, params: LookupField) -> Tour:
        ...
        """
        lookup_fields = self._get_lookup_fields(params)
        return self.tour_manager.get(**lookup_fields)
    ...
    def create_new_tour(self, validated_data: dict) -> Tour:
        ...
        new_tour = self.tour_manager.create(
            name=name,
            slug=slugify(name),
    ...
    def update_tour(self, tour: Tour, validated_data: dict):
        ...
        tour.max_group_size = validated_data.pop(
