...
from ..models.tour_review import TourReview
...
class TourReviewCRUDService:
    ...
    def get_all_reviews_on_tour(
            self, tour_id: int
    ) -> tuple[int, list[TourReview]]:
        """ Возвращает все отзывы на тур """
        reviews = TourReview.objects.select_related(
            'user__profile', 'tour'
    ...
    def get_tour_review_by_id(self, review_id: int) -> TourReview:
        ...
        return TourReview.objects.select_related(
            'user__profile', 'tour'
        ).get(pk=review_id)

    def create_tour_review(
            self, review_data: dict[str, Union[str, int]]
    ) -> TourReview:
